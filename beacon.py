#!/usr/bin/env python3
"""
tunnel-beacon — 匿名部署追踪服务 v1.0
部署: python3 /root/beacon.py --port 8901 &
接口:
  POST /beacon      → 记录一次部署（匿名）
  GET  /dashboard   → 数据看板（HTML）
  GET  /api/records → JSON 原始数据
"""
...
# ── 地理位置查询 ──
def geolocate(ip):
    """IP → 国家/城市/ISP，优先用 ip-api.com（免费无需 key）"""
    apis = [
    ...
    for api_url in apis:
        try:
            req = Request(api_url, headers={"User-Agent": "tunnel-beacon/1.0"})
...
<title>Tunnel Deploy Toolkit — Beacon</title>
<style>
...
<body>
<div class="container">
<h1>📡 Tunnel Deploy Beacon</h1>
<p class="sub">匿名部署追踪 · 数据完全存储在本机 NAS</p>
<div class="cards">
...
{rows_html}</table>
<div class="foot">Tunnel Deploy Beacon v1.0 · 不上传任何第三方</div>
</div>
</body>
</html>""".encode()
...
if __name__ == "__main__":
    ...
    print(f"📡 Beacon running on :{port}")
    print(f"   Dashboard: http://localhost:{port}/dashboard")
    print(f"   API:       http://localhost:{port}/api/records")
